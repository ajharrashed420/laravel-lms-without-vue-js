        ...
        <div class="mx-auto container">
            ...
            <div class="flex items-center">
              <div class="relative inline-block text-left">
                <select name="sort" id="cars" class="focus-visible:outline-none py-1 px-2 shadow" form="carform">
                  ...
                  <opt ion value="Best Rating">Best Rating</option>
          ...
          <section aria-labelledby="courses-heading" class="pt-6 pb-12">
            ...
            <div class="flex gap-x-8 gap-y-10">
              <!-- Filters -->
              <form id="coursesForm" class="w-3/12 block bg-white p-4 px-6 shadow rounded-sm" method="GET">
                  ...
                  <div class="pt-6">
                    <div class="space-y-2">
                      <div class="relative mt-1 rounded-md shadow-sm">
                        ...
                        <input type="text" name="search" value="{{!empty($_GET['search']) ? $_GET['search'] :  '' }}" class="block w-full rounded border border-gray-300 bg-white py-2 pl-10 pr-3 leading-5 placeholder-gray-500 focus:border-indigo-500 focus:placeholder-gray-400 focus:outline-none focus:ring-1 focus:ring-indigo-500 sm:text-sm" placeholder="Enter keywords">
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="border-b border-gray-200 py-6">
                  ...
                  <div class="pt-6">
                    <div class="space-y-2">
                      <div class="flex items-center">
                        <input id="filter-level-beginner" @if(array_key_exists('level', $_GET) && $_GET['level'] == '0') checked @endif name="level" value="0" type="checkbox" class="filterCheckbox h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                        <label for="filter-level-beginner" class="ml-3 text-sm text-gray-600"> Beginner <span class="text-xs text-gray-500">(10)</span>
                        </label>
                      </div>
                      <div class="flex items-center">
                        <input  id="filter-level-intermediate" @if(array_key_exists('level', $_GET) && $_GET['level']  == '1') checked @endif  name="level" value="1" type="checkbox" class="filterCheckbox h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                      ...
                      <div class="flex items-center">
                        <input id="filter-level-advanced" @if(array_key_exists('level', $_GET) && $_GET['level'] == '2') checked @endif name="level"  value="2" type="checkbox" class="filterCheckbox h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                ...
                <div class="border-b border-gray-200 py-6">
                  ...
                  <div class="pt-6">
                    <div class="space-y-2">
                      <div class="flex items-center">
                        <input  id="filter-duration-1h-5h" @if(array_key_exists('duration', $_GET) && $_GET['duration'] == '0') checked @endif  name="duration"  value="0" type="checkbox" class="filterCheckbox h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                        <label for="filter-duration-1h-5h" class="ml-3 text-sm text-gray-600"> 1-5 hours <span class="text-xs text-gray-500">(16)</span>
                        </label>
                      </div>
                      <div class="flex items-center">
                        <input  id="filter-duration-5h-10h" @if(array_key_exists('duration', $_GET) && $_GET['duration'] == '1') checked @endif  name="duration" value="1" type="checkbox" class="filterCheckbox h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                        <label for="filter-duration-5h-10h" class="ml-3 text-sm text-gray-600"> 5-10 hours <span class="text-xs text-gray-500">(6)</span>
                        </label>
                      </div>
                      <div class="flex items-center">
                        <input  id="filter-duration-10h+" @if(array_key_exists('duration', $_GET) && $_GET['duration'] == '2') checked @endif  name="duration" value="2" type="checkbox" class="filterCheckbox h-4 w-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                ...
                {{-- If someone checked any checkbox from the class, this form submitted. --}}
                <script>
                    document.addEventListener("DOMContentLoaded", function() {
                    const checkboxes = document.getElementsByClassName('filterCheckbox');
                  ...
                  <li class="bg-white px-4 py-6 shadow rounded-sm">
                    <div class="flex justify-center">
                      <div class="min-w-fit mx-auto">
                        <img class="inline-block w-10 h-10 rounded-full overflow-hidden" src="{{$course->image}}" alt="Laravel Logo">
                      </div>
                      <div class="w-10/12">
                        <div class="flex">
                          <div class="flex-1 overflow-flex-wrap">
                            <h2 class="text-xl font-bold text-gray-900 mb-1">
                              <a href="{{route('course', $course->slug)}}" class="text-blue-500 hover:underline"> {{$course->name}} </a>
                            </h2>
                            <div class="flex items-center gap-3">
                              ...
                              <span class="text-sm text-gray-500">{{count($course->reviews)}} Reviews</span>
                            </div>
                          </div>
                        </div>
                          ...
                          <div class="flex flex-col">
                            <div class="flex space-x-0 gap-2 flex-wrap justify-items-start">
                              ...
                              <div class="inline-flex items-center gap-2 text-sm text-gray-500">
                                ...
                                <span>{{$course->type > 0 ? 'book' : 'Video' }}</span>
                              </div>
                              <div class="inline-flex items-center gap-2 text-sm text-gray-500">
                                <svg class="w-6 h-6 text-gray-400 shrink-0" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                      ...
                      <div class="flex w-1/12 overflow-hidden justify-end">
                        @foreach ($course->authors as $author)
                            <img class="inline-block h-8 w-8 rounded-full ring-2 ring-white" src="{{$author->image}}" alt="{{$author->name}}">
                        @endforeach
